...
行政事業レビューデータ（2014-2024年度）をRSシステム形式のCSVに変換・正規化するパイプライン

## 概要

このプロジェクトは、[行政事業レビュー](https://www.gyoukaku.go.jp/review/database/index.html)で公開されているデータを、[RSシステム](https://rssystem.go.jp/)と同様の複数のCSVファイルに分割・正規化するパイプラインです。

**対応データ:**
- **2014-2023年度**: Excel/ZIPファイルを12種類のテーブルに変換
- **2024年度**: RSシステム形式CSVデータの正規化処理

2014年から2023年までの10年分のデータを処理し、**12種類のテーブル**（基本情報、予算・執行、支出先、組織情報、政策・法令、評価、関連事業など）に分割して出力します。2024年度のRSシステム形式データについては、既存のCSV形式を正規化してデータ品質を向上させます。

## 特徴

- **11年分のデータ対応**: 2014-2024年度の行政事業レビューデータを処理
- **多様なデータ形式に対応**: Excel（1シート/2シート構成）、ZIP形式、RSシステムCSV形式
- **RS2024データ正規化**: 2024年度RSシステムデータの正規化処理に対応
  - ZIP自動解凍
  - 3段階データフロー（ZIP → raw → processed）
  - 全角/半角の統一、カタカナ表記の統一
...
├── data/
│   └── download/              # 元データ配置場所
│       ├── *.xlsx, *.zip      # 2014-2023年度データ
│       └── RS_2024/           # 2024年度RSシステムデータ
├── output/
│   ├── raw/                   # Stage1: Excel→CSV変換後 / ZIP解凍後
│   │   └── year_2024/         # 2024年度解凍データ
│   ├── normalized/            # Stage2: 正規化後
│   ├── processed/             # Stage3+: 最終処理済みデータ
│   │   ├── year_2014-2023/   # 2014-2023年度処理済み（12ファイル/年）
│   │   └── year_2024/         # 2024年度正規化済み（15ファイル）
│   └── schema/                # スキーマ定義
├── src/
│   ├── pipeline/              # パイプライン処理
│   └── utils/                 # ユーティリティ（正規化等）
...
│   └── normalize_rs2024.py    # RS2024データ正規化スクリプト
├── tests/                     # テストコード
├── config.py                  # 設定ファイル
└── main.py                    # エントリーポイント
```
...
**2014-2023年度データ（歴史データ）:**
- `data/download/` に過去データのExcel/ZIPファイルを配置

**2024年度データ（RSシステム形式）:**
- `data/download/RS_2024/` にRSシステムのZIPファイルを配置
...
**RS2024データフロー:**
```
data/download/RS_2024/*.zip (15ファイル)
  ↓ Step 1: ZIP解凍
output/raw/year_2024/*.csv (デバッグ用)
  ↓ Step 2: 正規化処理
output/processed/year_2024/*.csv (本番利用)
```
...
**処理ファイル数**: 15ファイル（RSシステム標準形式）

- **総レコード数**: 553,109行（ヘッダー込み）
- **総データ量**: 459MB（正規化後）
- **主要ファイル**:
  - 5-1_支出先_支出情報: 194,134行（60MB）
  - 3-1_効果発現経路_目標・実績: 118,020行（65MB）
  - 2-2_予算種別・歳出予算項目: 55,610行（16MB）
  - 2-1_予算・執行_サマリ: 37,982行（12MB）
  - 5-3_費目・使途: 33,987行（12MB）

**正規化処理の効果**:
- ファイルサイズ削減: 約2.1%（469MB → 459MB）
- 全角/半角の統一、カタカナ表記の統一
- Unicode正規化（NFKC形式）
- データクリーニング（不要な空白・制御文字の除去）
